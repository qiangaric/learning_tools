...
#define g_warning(...) _torctlmain_log(G_LOG_LEVEL_WARNING, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
...
static gchar *optLogLevel = "debug";
...
{
  if ((globalIncludeGtidsArray && isGtidEventInGtidSetInfoArray(gtidEvent, globalIncludeGtidsArray) || (globalIncludeGtidsHash && isGtidEventInGtidSetInfoHashTable(gtidEvent, globalIncludeGtidsHash))))
  {
    return TRUE;
  }
  return FALSE;
...
{
  if ((globalExcludeGtidsArray && isGtidEventInGtidSetInfoArray(gtidEvent, globalExcludeGtidsArray) || (globalExcludeGtidsHash && isGtidEventInGtidSetInfoHashTable(gtidEvent, globalExcludeGtidsHash))))
  {
    return TRUE;
  }
  return FALSE;
...
gboolean isLeastExecutionUnitShouldKeep(LeastExecutionUnitEvents *leastExecutionUnitEvents)
{
  ...
  myloginfo("Starting torctl program on host  process id %i", (gint)getpid());
...
int setLogHandler()
{
  ...
  {
    g_log_set_handler(G_LOG_DOMAIN, G_LOG_LEVEL_MASK | G_LOG_LEVEL_DEBUG | G_LOG_LEVEL_INFO, g_log_default_handler, NULL);
  }
